...
async function main () {
  ...
  const isNewRelease = firstRelease || previousVersion !== newVersion
  const isNextRelease = newVersion === 'next'

  // 3. If versions are different or if its the first release
  if (isNewRelease || isNextRelease) {
    ...
    try {
      if (!isNewRelease) {
        const { data } = octokit.rest.repos.getReleaseByTag({
          owner,
          repo,
          tag: newVersion,
        })
      ...
      const { data } = release_id ? await octokit.repos.updateRelease(requestOptions) : await octokit.repos.createRelease(requestOptions)
